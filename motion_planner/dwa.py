...
from utils.variables import ObservationKeys
...
class DWA(BasePolicy):
    ...
    def convert_lidar_to_vertices(self, lidar, pose):
        ...
        # plt.show()
        return np.transpose(np.array([global_xs, global_ys]))
    ...
    def step(self, obs):
        ...
        """
        goal = obs[ObservationKeys.goal]
        pose = obs[ObservationKeys.pose]
        velocity = obs[ObservationKeys.velocity]
        lidar = obs[ObservationKeys.lidar]
        position = [pose[0], pose[1], pose[-1], 0, 0]
        vertices = self.convert_lidar_to_vertices(pose=pose, lidar=lidar * 10)
        dw = self.get_dynamic_window(current_v=velocity[0], current_a=velocity[1])
        return self._predict(x=position, u=velocity, dw=dw, ob=vertices, goal=goal)
